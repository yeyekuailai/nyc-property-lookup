...
const I18N = {
  en: {
    addressLabel: 'Step 1: Find the property by address', addressPlaceholder: 'Example: 20-28 154th St, Flushing, NY 11357', searchButton: 'Search Property',
    addressHelp: 'Step 2 shows the county and uses Borough, Block, and Lot for NOPV, occupancy certificate, and HPD searches.',
    bblLabel: 'Already have a BBL? Search directly', bblPlaceholder: 'Example: 3-6264-77 or 3062640077', bblButton: 'Search by BBL',
    searching: 'Searching NYC records…', complete: 'Search complete · Downstream searches use Borough, Block, and Lot', failed: 'Search failed',
  ...
  zh: {
    addressLabel: '第一步：用地址定位地块', addressPlaceholder: '例如 20-28 154th St, Flushing, NY 11357', searchButton: '查询物业',
    addressHelp: '第二步显示 County，并统一使用 Borough、Block、Lot 查询 NOPV、入住许可证和 HPD 记录。',
    bblLabel: '已有 BBL？直接查询', bblPlaceholder: '例如 3-6264-77 或 3062640077', bblButton: '用 BBL 查询',
    searching: '正在查询市府资料…', complete: '查询完成 · 后续查询统一使用 Borough、Block、Lot', failed: '查询失败',
...
async function findAddress(address) {
  ...
  const unique = [...new Map(matches.map(x => [x.bbl, x])).values()]; if (!unique.length) throw new Error(t('noMatch')); if (unique.length > 1) return { candidates: unique }; return propertyForBbl(unique[0].bbl, unique[0].label);
}
...
async function search(task) {
  ...
  try { const data = task.bbl ? await propertyForBbl(parseBbl(task.bbl) || (() => { throw new Error(t('badBbl')); })()) : await findAddress(task.address.trim()); currentData = data; render(data); statusState = 'complete'; status.textContent = t('complete'); }
